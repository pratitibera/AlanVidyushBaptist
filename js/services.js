	...
	for (i = 0; i < mainServicesData.length; i++) {
		...
		mainServices_section.innerHTML += `<div class="col-sm-4">
               <div class="single-service-item">
                  <div class="img-holder" id="holder_${i}" onmouseover="addzindex(this.id);" onmouseout="removezindex(this.id);">
                     ...
                     <div class="text-holder text-center" id="textholder_${i}">
                        <h3>${mainServicesData[i]['service']}</h3>
                        <p>${mainServicesData[i]['description']}</p>
                        <div class="thm-btn bgclr-1 w-50 ml-auto mr-auto mt-3" id="service_${i}" onclick="mainServiceDetails(this.id);">
...
function addzindex(item){
	$('#text' + item).addClass('index_99999');
}
function removezindex(item){
	document.getElementById('kioo').setAttribute('style', 'z-index: 9');
}
...
function getPlans() {
	...
	request.onload = function () {
		...
		for (j = 0; j < data['offers'].length; j++) {
			...
			duration.setAttribute('class', 'fo-22 fw-600 pco text-center pri_offer_heading');
			...
			price.setAttribute('class', 'text-center fw-600 fo-22');
			if (data['offers'][j]['discounted_price'] != undefined) {
				price.innerHTML = `<span class="fo-20" style="text-decoration: line-through">₹ ${data['offers'][j]['price']}</span>
                        <span class="pco fo-26 fw-600">₹ </span>${data['offers'][j]['discounted_price']}`;
			} else {
				price.innerHTML = `<span class="pco fo-26 fw-600">₹ </span>${data['offers'][j]['price']}`;
			}
			card.append(price);
